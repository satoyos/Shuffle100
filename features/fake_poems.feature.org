## このテストはうまく機能しない
##  stepsの中で、switchのon/offを無理やり設定しているが、それがScreenに認識されない。
...
Feature:
  ...
  空札を「on」に設定できる。
    ...
    # 空札スイッチをonに設定
    When I flip switch "空札を加える" on # <= これがScreenに認識されない。
  ...
  空札を「off」に設定できる。
    Then I should see "百首読み上げ"
      ...
      # 空札スイッチをonに設定
    When I flip switch "空札を加える" off # <= これがScreenに認識されない。
    Then switch "空札を加える" should be off
